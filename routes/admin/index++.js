    ...
    // validation.passes(async () => {
      ...
      //if user gives 50% off discount,
        ...
        for (const file of files) {
            let random=Date.now()
            let temp_path='uploads/uploaded'+random+'.jpg'

          const tmp_file = file.tempFilePath;
          
          jimp.read(tmp_file,async(err,img)=>{
              if(err){
                  console.log(err)
              }
              //height and width
              img.resize(1642,1532)
             .write(temp_path)

          const newPath = await uploader(temp_path);
         fs.unlinkSync(tmp_file);

        urls.push(newPath);
        console.log(urls)
      //  let rootPath = path.dirname(require.main.filename)
        //let combineAbsoluteAndRoot=path.join(rootPath,temp_path)
  })
          //const newPath = await uploader(tmp_file);
          //urls.push(newPath);
         // fs.unlinkSync(tmp_file);
